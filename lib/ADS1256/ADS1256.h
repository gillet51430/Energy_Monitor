...
public:
    ...
    void begin();
    void setPGA(uint8_t pga_setting);
    void setDataRate(uint8_t drate_setting);
    void setChannel(uint8_t positive_input, uint8_t negative_input);
    ...
    void startContinuousConversion(uint8_t positive_input, uint8_t negative_input);
    void stopContinuousConversion();
    long readContinuousData_LSB();
    // double readVoltageDifferential_AIN0_AIN1();
    double readVoltageDifferential(uint8_t positive_input, uint8_t negative_input);
    double convertToVolts(long raw_data);
...
private:
    ...
    void calculateLSBResolution();
    void updatePGAGainValue();
};

#endif // ADS1256_PERSO_H